<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
<html>
  <!--
  Generated from screenshots.xml on Wed Mar 13 08:23:38 2002
  -->
  <head>
    <title>Screenshots</title>
  <style type="text/css"><!--
    select.theme {
	background-color:#fff;
	border:1px #666666 solid;
	font-size: 10px;
	color: #666666;
	font-family: arial,sans-serif;
    }
    input.theme {
	font-size: 10px;
	font-family: arial,sans-serif;
	margin-top: 1px;
	margin-bottom: 1px;
    }
    a {
	text-decoration: none;
	color: yellow;
    }
    a:hover {
	text-decoration: underline;
	color: white;
    }
    body {
	margin: 0;
	background-color: #464646;
    }
    h1 {
	color: white;
    }
    h2 {
	color: yellow;
	background-color: #666666;
    }
    h6 {
	font-family: western, fantasy;
	color: #888888;
    }
    tbody {
	color: white;
    }
    td.contents {
	border-style: ridge;
	padding: 2;
    }
    .table {
	border-style: ridge;
	padding: 2;
    }
  --></style>
<link REL="icon" HREF="/bookmark.ico" TYPE="image/png">
  </head>

  <body link="yellow" vlink="yellow" text="yellow" bgcolor="#464646">
    <table border="0" width="100%" cellspacing="0" cellpadding="0" align="center">
      <tr>
        <td width="100%">
          <table border="0" width="100%" cellspacing="0" cellpadding="0">
            <tr>
              <td width="390"><img border="0" src="images_modern/top-left.gif" width="390" height="108" alt=""></td>
              <td align="left"><h1>Screenshots</h1></td>
            </tr>
          </table>
        </td>
      </tr>

      <tr>
        <td width="100%">
          <table border="0" width="100%" cellspacing="0" cellpadding="0">
            <tr>
              <td valign="top">
                <!-- navbar link -->
                <table border="0" cellspacing="0" cellpadding="0" width="100%">
                  <tr>
                    <td colspan="3"><img src="images_modern/button1_nw.gif" width="120" height="8" alt=""><td>
                  </tr>
                  <tr>
                    <td><img src="images_modern/button1_w.gif" width="71" height="30" alt=""></td>
                    <td bgcolor="#646464" align="right" width="100%">
                    <a href="index.php">Index</a></td>
                    <td><img src="images_modern/button1_e.gif" width="14" height="30" alt=""></td>
                  </tr>
                  <tr>
                    <td colspan="3"><img src="images_modern/button1_sw.gif" width="120" height="12" alt=""></td>
                  </tr>
                </table>

                <!-- navbar link -->
                <table border="0" cellspacing="0" cellpadding="0" width="100%">
                  <tr>
                    <td colspan="3"><img src="images_modern/button1_nw.gif" width="120" height="8" alt=""><td>
                  </tr>
                  <tr>
                    <td><img src="images_modern/button1_w.gif" width="71" height="30" alt=""></td>
                    <td bgcolor="#646464" align="right" width="100%">
                    <a href="download.php">Download</a></td>
                    <td><img src="images_modern/button1_e.gif" width="14" height="30" alt=""></td>
                  </tr>
                  <tr>
                    <td colspan="3"><img src="images_modern/button1_sw.gif" width="120" height="12" alt=""></td>
                  </tr>
                </table>

                <!-- navbar link -->
                <table border="0" cellspacing="0" cellpadding="0" width="100%">
                  <tr>
                    <td colspan="3"><img src="images_modern/button1_nw.gif" width="120" height="8" alt=""><td>
                  </tr>
                  <tr>
                    <td><img src="images_modern/button1_w.gif" width="71" height="30" alt=""></td>
                    <td bgcolor="#646464" align="right" width="100%">
                    <a href="contact.php">Contact</a></td>
                    <td><img src="images_modern/button1_e.gif" width="14" height="30" alt=""></td>
                  </tr>
                  <tr>
                    <td colspan="3"><img src="images_modern/button1_sw.gif" width="120" height="12" alt=""></td>
                  </tr>
                </table>

                <!-- navbar link -->
                <table border="0" cellspacing="0" cellpadding="0" width="100%">
                  <tr>
                    <td colspan="3"><img src="images_modern/button1_nw.gif" width="120" height="8" alt=""><td>
                  </tr>
                  <tr>
                    <td><img src="images_modern/button1_w.gif" width="71" height="30" alt=""></td>
                    <td bgcolor="#646464" align="right" width="100%">
                    <a href="links.php">Links</a></td>
                    <td><img src="images_modern/button1_e.gif" width="14" height="30" alt=""></td>
                  </tr>
                  <tr>
                    <td colspan="3"><img src="images_modern/button1_sw.gif" width="120" height="12" alt=""></td>
                  </tr>
                </table>

                <!-- navbar link -->
                <table border="0" cellspacing="0" cellpadding="0" width="100%">
                  <tr>
                    <td colspan="3"><img src="images_modern/button1_nw.gif" width="120" height="8" alt=""><td>
                  </tr>
                  <tr>
                    <td><img src="images_modern/button1_w.gif" width="71" height="30" alt=""></td>
                    <td bgcolor="#646464" align="right" width="100%">
                    <a href="changelog.php">Changelog</a></td>
                    <td><img src="images_modern/button1_e.gif" width="14" height="30" alt=""></td>
                  </tr>
                  <tr>
                    <td colspan="3"><img src="images_modern/button1_sw.gif" width="120" height="12" alt=""></td>
                  </tr>
                </table>

                <!-- navbar stuff -->
                <table border="0" cellspacing="0" cellpadding="0" width="120">
                  <tr>
                    <td align="center" bgcolor="#5b5b5b">
                      <p><a href="http://www.guistuff.com"><img border="0" src="images_modern/gui.gif" width="105" height="78" alt="guistuff"></a></p>
		      <p><a href="http://sourceforge.net"><img SRC="http://sourceforge.net/sflogo.php?group_id=2599&amp;type=1" BORDER=0 height=31 width=88 ALT="Sourceforge Logo"></a></p>
                      <p><a href="http://validator.w3.org/check/referer"><img border="0" src="valid-html401.gif" alt="Valid HTML 4.01!" height="31" width="88"></a></p>
		      <form action="<?php echo $PHP_SELF ?>" method="POST">
		      <select name="sel_theme" class="theme" title="Select your theme here">
		      <option value="simple" <?php if ($theme=="simple") echo "selected" ?>>Simple</option>
		      <option value="carsten" <?php if ($theme=="carsten") echo "selected" ?>>Carsten</option>
		      <option value="modern" <?php if ($theme=="modern") echo "selected" ?>>Modern</option>
		      </select>
		      <input type="submit" name="Submit" value="Change" class="theme">
		      </form>
                    </td>
                  </tr>
                </table>
              </td>
              <td class="contents" width="100%" valign="top"><h2>Appearance</h2><center><a href="screenshots/new/full.jpg"><img src="screenshots/new/full_s.jpg" alt="Appearance" width="387" height="335" border="0"></a></center><h2>The controls</h2><center><a href="screenshots/new/controls.jpg"><img src="screenshots/new/controls_s.jpg" alt="Controls" width="225" height="250" border="0"></a></center><h2>The preferences</h2><center><a href="screenshots/new/preferences.jpg"><img src="screenshots/new/preferences_s.jpg" alt="Preferences" width="287" height="173" border="0"></a></center><h2>Some installed plugins</h2><center><a href="screenshots/new/plugins.jpg"><img src="screenshots/new/plugins_s.jpg" alt="Installed Plugins" width="254" height="219" border="0"></a></center><h2>The channel editor</h2><center><a href="screenshots/new/channels.jpg"><img src="screenshots/new/channels_s.jpg" alt="Channel Editor" width="290" height="177" border="0"></a></center>              </td>
            </tr>
          </table>
        </td>
      </tr>
    </table>
  </body>
</html>
