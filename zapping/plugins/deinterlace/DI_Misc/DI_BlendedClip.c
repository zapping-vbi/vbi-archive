/////////////////////////////////////////////////////////////////////////////
// $Id: DI_BlendedClip.c,v 1.3 2005-06-28 00:49:04 mschimek Exp $
/////////////////////////////////////////////////////////////////////////////
...
BOOL DeinterlaceBlendedClip(TDeinterlaceInfo* pInfo)
{
    ...
    const __int64 SomeOnes  = 0x0001000100010001LL;   
    __int64 i;
    __int64 MinClip;
    ...
    if (BlcBlendChroma)
    {
        if (BlcUseInterpBob)        // use Luma, chroma from interp bob 
        {
            L1Mask = 0;
            LCMask = 0;
            BobMask = 0xfefffefffefffeffLL;
        }
        else                        // use Luma, chroma from Clip
        {
            L1Mask = 0;
            LCMask = 0xffffffffffffffffLL;
            BobMask = 0;
        }
    }
    else
...
"            mov esi, %[LP1]        \n"
"            mov ecx, %[LP3]   \n"
"            mov edi, %[Dest]       ## DL2 if Odd or DL1 if Even \n"
...
"            movq    %[MotionAvg], mm0\n"
_asm_end,
_m(L1), _m(L2), _m(L3), _m(LP1), _m(LP3), _m(Dest), _m(YMask),
_m(ShiftMask), _m(BobMask), _m(LCMask), _m(CombAvgL), _m(PixelCombSense),
_m(MinClipMinus), _m(MotionAvgL), _m(PixelMotionSense), _m(MinClip),
_m(SomeOnes), _m(UVMask), _m(L1Mask), _m(LoopCtr), _m(CombAvg), _m(MotionAvg)
: "eax", "ecx", "edx", "esi", "edi");
