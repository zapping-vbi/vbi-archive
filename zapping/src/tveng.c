...
*/
#define TVENG_BTTV_MUTE_BUG_WORKAROUND 1
...
#ifdef TVENG_BTTV_MUTE_BUG_WORKAROUND
  ...
  if (info -> fd < 0)
    {
      ...
      snprintf(info -> error, 256, _("Cannot open %s"), info->file_name);
      return -1;
    }
  ...
  if (ioctl(info -> fd, VIDIOCGCAP, &caps))
    {
      ...
      perror("VIDIOCGCAP");
      snprintf(info -> error, 256, _("VIDIOCGCAP failed: %d"), info ->
	       tveng_errno);
      close(info -> fd);
      return -1;
    }
			...
			tveng_device_info * info)
{
  ...
  if (info -> fd) /* If the device is already attached, deattach it */
    tveng_close_device(info);
  ...
  info -> fd = tveng_open_device_file(flags, info);
  ...
  */
  ...
  if (tveng_set_input(&(info->inputs[0]), info) == -1)
    {
      tveng_close_device(info);
      return -1;
    }
  ...
  info->standards = NULL;
  info->cur_standard = 0;
  error = tveng_get_standards(info);
  if (error < 1)
    {
  ...
  if (tveng_set_standard(&(info->standards[0]), info) == -1)
    {
      tveng_close_device(info);
      return -1;
    }
  ...
  info->num_controls = 0;
  info->controls = NULL;
  error = p_tveng_build_controls(info);
  if (error == -1)
      return -1;
  ...
  switch(error)
    ...
    default:
      info -> tveng_errno = -1;
      fprintf(stderr, "error on switching %d\n", error);
      t_error_msg("switch()", 
...
int tveng_get_inputs(tveng_device_info * info)
{
  ...
  if (i) /* If there is any channel, switch to the first one */
      ...
      if (ioctl(info->fd, VIDIOCGCHAN, &channel))
	{
	  info -> tveng_errno = errno;
	  perror("VIDIOCGCHAN");
	  snprintf(info -> error, 256, _("Cannot get input 0\n"));
	  return -1;
	}
      if (ioctl(info->fd, VIDIOCSCHAN, &channel))
	{
	  info -> tveng_errno = errno;
	  perror("VIDIOCSCHAN");
	  snprintf(info -> error, 256, _("Cannot set input 0\n"));
	  return -1;
	}
    }
...
*/
int tveng_set_input(struct tveng_enum_input * input, tveng_device_info * info)
{
  enum tveng_capture_mode current_mode = info -> current_mode;
  struct video_channel channel;
  ...
  switch (info -> current_mode)
	...
	return -1;
      break;
    case TVENG_CAPTURE_PREVIEW:
      if (tveng_stop_previewing(info) == -1)
	return -1;
      break;
    default:
      break;
  ...
  if (ioctl(info->fd, VIDIOCGCHAN, &channel))
    {
      info->tveng_errno = errno;
      perror("VIDIOCGCHAN");
      snprintf(info->error, 256, _("Getting input failed: %d"),
	       info->tveng_errno);
      return -1;
    }    
  ...
  if (ioctl(info->fd, VIDIOCSCHAN, &channel))
    {
      info->tveng_errno = errno;
      perror("VIDIOCSCHAN");
      snprintf(info->error, 256, _("Setting input failed: %d"),
	       info->tveng_errno);
      return -1;
    }
...
int tveng_get_standards(tveng_device_info * info)
{
  ...
  struct dummy_standard_struct std_t[] = /* The standards supported by
					  bttv, the v4l definition is
					  somewhat different */
  {
    {  0, "PAL" },
    {  1, "NTSC" },
...
int tveng_set_standard(struct tveng_enumstd * std, tveng_device_info * info)
{
  ...
  if (ioctl(info->fd, VIDIOCGCHAN, &channel))
    {
      info->tveng_errno = errno;
      snprintf(info->error, 256, _("Getting standard failed: %d"),
	       info->tveng_errno);
      perror("VIDIOCGCHAN");
      return -1;
    }    
  ...
  if (ioctl(info->fd, VIDIOCSCHAN, &channel))
    {
      info->tveng_errno = errno;
      snprintf(info->error, 256, _("Setting standard failed: %d"),
	       info->tveng_errno);
      perror("VIDIOCSCHAN");
      return -1;
    }
  ...
  if (ioctl(info->fd, VIDIOCGWIN, &window))
    {
      info->tveng_errno = errno;
      perror("VIDIOCGWIN");
      snprintf(info->error, 256, _("Cannot get window attributes"));
      return -1;
    }
...
#define P_TVENG_C_AUDIO_MUTE 0
#define P_TVENG_C_AUDIO_VOLUME 1
#define P_TVENG_C_AUDIO_BASS 2
#define P_TVENG_C_AUDIO_TREBLE 3
#define P_TVENG_C_AUDIO_BALANCE 4
#define P_TVENG_C_AUDIO_MIN P_TVENG_C_AUDIO_MUTE /* the first audio control */
#define P_TVENG_C_AUDIO_MAX P_TVENG_C_AUDIO_BALANCE /* the last audio
						 control */
#define P_TVENG_C_VIDEO_BRIGHTNESS 100 /* leave free room */
#define P_TVENG_C_VIDEO_HUE 101
#define P_TVENG_C_VIDEO_COLOUR 102
#define P_TVENG_C_VIDEO_CONTRAST 103
#define P_TVENG_C_VIDEO_MIN P_TVENG_C_VIDEO_BRIGHTNESS /* the first video
						    control */
#define P_TVENG_C_VIDEO_MAX P_TVENG_C_VIDEO_CONTRAST /* The last video
						  control */
		       ...
		       tveng_device_info * info)
{
  ...
  if (!new_pointer)
    {
      info->tveng_errno = errno;
      snprintf(info->error, 256, _("CRITICAL:: Cannot allocate enough mem"));
      perror("realloc");
      return -1;
    }
...
p_tveng_build_controls(tveng_device_info * info)
{
  ...
  if (ioctl(info->fd, VIDIOCGAUDIO, &audio))
    {
      info->tveng_errno = errno;
      perror("VIDIOCGAUDIO");
      snprintf(info->error, 256, _("Cannot get audio info"));
      return -1;
    }
  if (ioctl(info->fd, VIDIOCGPICT, &pict))
    {
  ...
  if (audio.flags & VIDEO_AUDIO_MUTABLE)
    {
      control.id = P_TVENG_C_AUDIO_MUTE;
  ...
  if (audio.flags & VIDEO_AUDIO_VOLUME)
    {
      control.id = P_TVENG_C_AUDIO_VOLUME;
  ...
  if (audio.flags & VIDEO_AUDIO_BASS)
    {
      control.id = P_TVENG_C_AUDIO_BASS;
  ...
  if (audio.flags & VIDEO_AUDIO_TREBLE)
    {
      control.id = P_TVENG_C_AUDIO_TREBLE;
  ...
  control.id = P_TVENG_C_AUDIO_BALANCE;
  ...
  /* FIXME: Add the option for changing the audio source: Mono,
     stereo, European 1 and 2 */

  /* Build the video controls now */
  control.id = P_TVENG_C_VIDEO_BRIGHTNESS;
  ...
  control.id = P_TVENG_C_VIDEO_HUE;
  ...
  control.id = P_TVENG_C_VIDEO_COLOUR;
  ...
  control.id = P_TVENG_C_VIDEO_CONTRAST;
  ...
  return (tveng_update_controls(info)); /* Fill in with the valid
					   values */
}
...
#endif
	  break;
	case P_TVENG_C_AUDIO_VOLUME:
	  control -> cur_value = audio.volume;
	  break;
	case P_TVENG_C_AUDIO_BASS:
	  control -> cur_value = audio.bass;
	  break;
	case P_TVENG_C_AUDIO_TREBLE:
	  control -> cur_value = audio.treble;
	  break;
	case P_TVENG_C_AUDIO_BALANCE:
	  control -> cur_value = audio.balance;
	  break;
	/* Video controls */
	case P_TVENG_C_VIDEO_BRIGHTNESS:
	  control -> cur_value = pict.brightness;
	  break;
	case P_TVENG_C_VIDEO_HUE:
	  control -> cur_value = pict.hue;
	  break;
	case P_TVENG_C_VIDEO_COLOUR:
	  control -> cur_value = pict.colour;
	  break;
	case P_TVENG_C_VIDEO_CONTRAST:
	  control -> cur_value = pict.contrast;
	  break;
	default:
	  info->tveng_errno = -1;
	  snprintf(info->error, 256, _("Unknown control: %s (%d)"),
		  ...
		  tveng_device_info * info)
{
  ...
  /* we are supposing that P_TVENG_C_AUDIO_MIN < P_TVENG_C_VIDEO_MAX
     and that P_TVENG_C_VIDEO_MIN > P_TVENG_C_AUDIO_MAX */
  t_assert(control->id >= P_TVENG_C_AUDIO_MIN);
  t_assert(control->id <= P_TVENG_C_VIDEO_MAX);
  t_assert((control->id >= P_TVENG_C_VIDEO_MIN) || (control->id <=
	   P_TVENG_C_AUDIO_MAX));
  ...
  if (control -> id <= P_TVENG_C_AUDIO_MAX) /* Set an audio control */
    {
      if (ioctl(info->fd, VIDIOCGAUDIO, &audio))
	{
	  info->tveng_errno = errno;
      ...
      if (ioctl(info->fd, VIDIOCSAUDIO, &audio))
	{
	  info->tveng_errno = errno;
	  perror("VIDIOCSAUDIO");
	  snprintf(info->error, 256, _("Cannot set audio info"));
	  return -1;
	}
    }
      ...
      if (ioctl(info->fd, VIDIOCGPICT, &pict))
	{
	  info->tveng_errno = errno;
	  perror("VIDIOCGPICT");
	  snprintf(info->error, 256, _("Cannot get video info"));
	  return -1;
	}
      switch(control->id)
	{
      ...
      if (ioctl(info->fd, VIDIOCSPICT, &pict))
	{
	  info->tveng_errno = errno;
	  perror("VIDIOCSPICT");
	  snprintf(info->error, 256, _("Cannot set video info"));
	  return -1;
	}
    }
  /* updates the value of all the controls */
  return (tveng_update_controls(info));
}
			  ...
			  tveng_device_info * info)
{
  ...
  /* if we reach this, we haven't found the mute control (!!??!!) */
  info->tveng_errno = -1;
  snprintf(info->error, 256, 
...
tveng_tune_input(__u32 freq, tveng_device_info * info)
{
  ...
  if (ioctl(info->fd, VIDIOCGTUNER, &tuner))
    {
      ...
      perror("VIDIOCGTUNER");
      snprintf(info->error, 256, 
      _("Cannot get info about the first tuner of the current input"));
      return -1;
    }
  ...
  /* Ok, tune the current input (FIXME: we are assuming all the time
     that we tuning the first tuner of the first channel) */
  if (ioctl(info->fd, VIDIOCSFREQ, &new_freq))
    {
      info->tveng_errno = errno;
      perror("VIDIOCSFREQ");
      snprintf(info->error, 256, 
      _("Cannot tune the device"));
      return -1;
    }
  return 0; /* Success */
}
...
tveng_get_tune(__u32 * freq, tveng_device_info * info)
{
  ...
  if (info->inputs[info->cur_input].tuners == 0)
    {
      *freq = 0; /* Doesn't matter */
      return 0; /* Success (we shouldn't be tuning, anyway) */
    }

  /* get the current tune value (FIXME: we are assuming all the time
     that we tuning the first tuner of the first channel) */
  if (ioctl(info->fd, VIDIOCGFREQ, &real_freq))
    {
      ...
      perror("VIDIOCFFREQ");
      snprintf(info->error, 256, 
      _("Cannot get the current tuning freq"));
      return -1;
    }
...
{
  if (tveng_update_capture_format(info))
    return -1;
  ...
  if (ioctl(info -> fd, VIDIOCGFBUF, &buffer))
    {
      info->tveng_errno = errno;
      perror("VIDIOCGFBUF");
      snprintf(info->error, 256, _("Cannot get framebuffer info"));
      return 0;
    }
...
tveng_run_zapping_setup_fb(tveng_device_info * info)
{
  ...
  if (pid == -1)
    {
      info->tveng_errno = errno;
      perror("fork()");
      snprintf(info->error, 256, _("Cannot fork"));
      return -1;
    }
  ...
  if (waitpid(pid, &status, 0) == -1)
    {
      ...
      perror("waitpid");
      snprintf(info->error, 256, 
	       _("Something weird happened to the child proccess"));
      return -1;
    }
  ...
  switch (WEXITSTATUS(status))
    ...
    case 1:
      info -> tveng_errno = -1;
      snprintf(info->error, 256,  
	      _("zapping_setup_fb failed to set up the video"));
      return -1;
    case 2:
      info -> tveng_errno = -1;
      snprintf(info->error, 256,
...
tveng_get_display_depth(tveng_device_info * info)
{
  ...
  if (v == -1) {
    fprintf(stderr, _("Cannot find an appropiate visual"));
    return 0;
  }
  ...
  if (bpp == 0) {
    fprintf(stderr,_("Cannot figure out X depth"));
    return 0;
  }
  return bpp;
}
...
tveng_set_preview_window(tveng_device_info * info)
{
  ...
  if (v4l_window.clipcount)
      ...
      if (!clips)
	{
	  info->tveng_errno = errno;
	  perror("malloc");
	  snprintf(info->error, 256, _("CRITICAL:: Not enough mem"));
	  return -1;
	}
  ...
  if (ioctl(info->fd, VIDIOCSWIN, &v4l_window))
    {
      ...
      if (clips)
	free(clips);
      snprintf(info->error, 256, _("Cannot set new window parameters"));
      return -1;
    }
  ...
  return (tveng_get_preview_window(info));
}
...
tveng_get_preview_window(tveng_device_info * info)
{
  /* Updates the entire capture format, since in V4L there is no
     difference */
  return (tveng_update_capture_format(info));
}
  ...
  if (ioctl(info->fd, VIDIOCCAPTURE, on ? &one : &zero))
    {
      ...
      snprintf(info->error, 256, _("Cannot begin previewing"));
      return -1;
    }
  return 0;
}
...
tveng_start_previewing (tveng_device_info * info)
{
  ...
  if (!tveng_detect_XF86DGA(info))
    {
      info->tveng_errno = -1;
      snprintf(info->error, 256, 
	       _("The X server doesn't report any DGA extension"));
      fprintf(stderr, "tveng_detect_XF86DGA failed\n");
      return -1;
    }
  ...
  if (tveng_set_preview_window(info) == -1)
    {
