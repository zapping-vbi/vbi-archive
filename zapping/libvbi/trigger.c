...
parse_eacem(vbi_trigger *t, unsigned char *s1, unsigned int nuid, double now)
{
	...
	if (strncmp(t->link.url, "http://", 7) == 0)
		...
		t->link.page = parse_dec(t->link.url + 5, 2);
		if (!t->link.name || t->link.page < 0 || t->link.url[7])
			return NULL;
		...
		t->link.page = parse_hex(t->link.url + 11, 3);
		if (t->link.page < 0x100 || t->link.url[14] != '/')
			return NULL;

		t->link.subpage = parse_hex(t->link.url + 15, 4);
		if (t->link.subpage < 0)
			return NULL;
...
void
vbi_trigger_flush(struct vbi *vbi)
{
	vbi_trigger *t;
...
void
vbi_deferred_trigger(struct vbi *vbi)
{
	vbi_trigger *t, **tp;
...
static void
add_trigger(struct vbi *vbi, vbi_trigger *a)
{
	vbi_trigger *t;
...
void
vbi_eacem_trigger(struct vbi *vbi, unsigned char *s)
{
	vbi_trigger t;
